...
1254 × 1254, generated by `scripts/generate-icons.js`). Shown on the dying player's own screen
only, for the full 3 s respawn delay; other players just see the kill feed. Beats, timed from
the death tick: **0–0.3 s** the local render drops to slow motion (0.25× interpolation, the
host simulation is untouched), desaturates to grey with a red-brown tint under a dark vignette,
and the camera pushes in on the body; the own-death haptic fires here. **0.3 s** the artwork
slams in over the whole viewport, scaling 1.15× → 1× with a short overshoot; it is always shown
complete (`object-fit: contain` on a near-black backdrop, a blurred copy of the same art filling
the letterbox on wide screens). **2.6–3.0 s** fade to black, then the respawn cuts back to
