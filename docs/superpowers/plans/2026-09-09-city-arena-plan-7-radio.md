...
- [ ] **Step 5: Generate** — `npx dotenv -e C:/Users/Guido/Projects/H3-Teamy/.env -- npm run -s arena:generate-radio`, then `ffprobe` each file (mp3, 44.1 kHz, ~120 s) and `npm run -s arena:check-audio`. If the API answers 4xx about the model, set `MODEL_ID` to `music_v1` and rerun; if it refuses the plan, stop and report — the code paths are all tested against a fake dial and merge without tracks.
