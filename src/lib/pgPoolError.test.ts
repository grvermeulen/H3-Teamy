...
describe("isPgPoolIdleDisconnectNoise", () => {
  ...
  it("returns true for Connection terminated (pg ending path)", () => {
    expect(isPgPoolIdleDisconnectNoise(new Error("Connection terminated"))).toBe(
      true,
    );
  });

  it("returns false for unrelated pool errors", () => {
    expect(isPgPoolIdleDisconnectNoise(new Error("password authentication failed"))).toBe(
      false,
    );
  });
