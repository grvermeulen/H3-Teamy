...
export async function GET(req: NextRequest) {
  ...
  for (const { userId, status } of items) {
    ...
    // Compute simple attendance percentage as Yes/(Yes+No+Maybe) across all events
    let percent = 0;
    try {
      const history = await listUserRsvps(userId);
      const total = history.length;
      const yesCount = history.filter((h) => h.status === "yes").length;
      percent = total > 0 ? (yesCount / total) * 100 : 0;
    } catch {}
    const badge = getBadgeForAttendance(percent);
    if (status === "yes") yes.push({ id: userId, name, badge });
    else if (status === "no") no.push({ id: userId, name, badge });
    else if (status === "maybe") maybe.push({ id: userId, name, badge });
  }
