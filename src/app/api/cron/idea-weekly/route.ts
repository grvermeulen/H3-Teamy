...
/**
 * Weekly Vercel-Cron entrypoint (registered in `vercel.json`, Mon 08:00 UTC).
 * Pulls the last 7 days of IDEA feedback, calls the AI Product Manager via the
 * Vercel AI Gateway to triage each new row (theme/impact/summary), persists
 * the enrichment, and posts a grouped Slack Block Kit digest. Authenticated
 * via the `Authorization: Bearer $CRON_SECRET` header that Vercel-Cron sets;
...
export async function GET(req: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    if (ideas.length === IDEA_CAP) {
      Sentry.captureMessage(
        "idea-weekly hit IDEA_CAP — some ideas not processed",
        {
          level: "warning",
    ...
    Sentry.captureException(err, { tags: { component: "idea-weekly" } });
