...
async function main() {
  ...
  try {
    ...
    if (keys.length === 0) {
      ...
      console.log("This suggests the data was never stored or has been deleted.");
      return;
    }
    ...
    console.log(`\n=== Summary ===`);
    console.log(`Total unique user IDs in attendance data: ${allAttendanceUserIds.size}`);
    ...
    if (datesWithData.length < sortedKeys.length) {
      console.log(`⚠️  ${sortedKeys.length - datesWithData.length} dates have no attendance data`);
    }
    ...
    for (const dateKey of sortedKeys.slice(0, 5)) {
      ...
      console.log(`${dateKey}: ${attUserIds.length} users (${valid} valid, ${orphaned} orphaned)`);
    }
  } catch (error) {
