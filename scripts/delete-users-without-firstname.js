...
async function main() {
  ...
  try {
    ...
    usersToDelete.slice(0, 10).forEach((user, i) => {
      const name =
        `${user.firstName || "(null)"} ${user.lastName || ""}`.trim() ||
        user.id.slice(0, 8);
      console.log(
        `  ${i + 1}. ${name} (${user.email || "no email"}) - ID: ${user.id}`,
      );
    });
    ...
    if (attendanceCount > 0 || rsvpCount > 0 || identityCount > 0) {
      ...
      if (identityCount > 0)
        console.log(`   - ${identityCount} identity records`);
    }
    ...
    if (dryRun) {
      ...
      console.log(
        `Total records that would be deleted: ${usersToDelete.length + attendanceCount + rsvpCount + identityCount}`,
      );
      return;
    }
